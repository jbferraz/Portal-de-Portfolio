...
class Cadastro extends CI_Controller {
    ...
    function index() 
    {
        ...
        $this->load->view('backend_test/cadastro/cadastro', $data);
    }
    ...
    function submit()
	{
        ...
        if ($fields['senha'] != hash('sha512', $this->input->post('senha2'))) {
            ...
            $this->session->set_flashdata('error_msg', 'Cadastro já existe');
                ...
                $result = $this->gn_m->submit('usuario', $fields);
                if ($result) {
                    try {
                        $n_user = $this->gn_m->
                            getGenericWhere('celular', $fields['celular'], 'usuario', null, null)[0];
                        $c_hash = 
                            getHash($fields['celular'], 0, $n_user->idusuario);
                        $hash = $this->gn_m->
                            submit('chave_hash', $c_hash);
                        $n_hash = $this->gn_m->
                            getGenericWhere('chave', "'".$c_hash['chave']."'", 'chave_hash', null, null)[0];
                        $c_rate = 
                            getAvaliacao($n_hash->idchave, $n_user->celular, 3, 1, $n_user->idusuario);
                        $rate = $this->gn_m->
